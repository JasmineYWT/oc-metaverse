...
只输出 JSON，不要有其他文字。`;
    ...
    async generateAILikes(postId) {
        ...
        // 分离 OC 角色和路人
        const ocIds = allIds.filter(i => {
            const chat = this.getChat(i.id);
            return chat && !chat.isNPC;
        });

        const npcIds = allIds.filter(i => {
            const chat = this.getChat(i.id);
            return !chat || chat.isNPC;
        });

        // OC 只随机取 1-3 个（如果 OC 不够就全取）
        const ocCount = Math.min(Math.floor(Math.random() * 3) + 1, ocIds.length);
        const selectedOCs = [...ocIds].sort(() => Math.random() - 0.5).slice(0, ocCount);

        // 其余全部用路人补充
        const remainingCount = Math.max(0, count - selectedOCs.length);
        const selectedNPCs = [...npcIds].sort(() => Math.random() - 0.5).slice(0, remainingCount);
