...
请用符合你性格的语气，生成一句推荐语（10字以内）：`;
    ...
    async shouldForwardForumPost(chat, post) {
        ...
        // 频率控制（复用动态转发频率滑块）
        const freq = this.mammySettings?.autoGenerate?.dynamics?.ocForwardFrequencies?.[chat.id] ?? 3;
        let forwardProbability = (freq / 10) * 0.5;   // 0.5 是衰减系数，可自行调整
    ...
    async forwardForumPostByCharacter(chat, post) {
        ...
        if (this.mammySettings?.apiUrl && this.mammySettings?.apiKey) {
            try {
                ...
                const intentPrompt = `你是${chat.nickname || chat.name}，${chat.personalityPrompt ? '你的性格：' + chat.personalityPrompt.substring(0, 150) : ''}。你转发这篇帖子给妈咪。妈咪是你的创造者，你们关系亲密，你对她说话可以撒娇、吐槽、调笑、嗔怪、分享，完全取决于你的性格。

帖子标题：《${truncatedTitle}》
帖子作者：${authorIdentity}
帖子摘要：${truncatedContent}

${authorId === 'user_mummy' ? '这篇帖子是妈咪自己发的。你可以调侃她、赞美她，或者说点俏皮话。' : ''}
${post.authorId && post.authorId.startsWith('writer_') ? '这篇帖子是写手太太写的同人文。你可以吐槽、调笑、反驳、害羞，或者幻想一下自己和配对角色真这样也不错。' : ''}
${authorIdentity.includes('配对角色') ? '这篇帖子是你的配对角色发的。你可能害羞、调情、傲娇、或者嘴上嫌弃心里开心。' : ''}

请用一句口语化的推荐语（10-25字），直接对妈咪说。可以带语气词（如"你看"、"哈哈"、"天哪"、"啧"、"哼"），像真人分享一样，**不要写"转发理由"之类的说明**。只输出推荐语本身。`;

                const result = await this.callAIForDynamic(intentPrompt, { temperature: 0.9,  reasoning_effort: "low" });
                if (result) {
        ...
        if (!forwardInfo.reason) {
            const fallbackReasons = [
                '你看看这个', '有点意思', '说得对', '我不理解', '哈哈哈',
                '这都什么啊', '必须分享', '写得好', '我无语了', '看看人家'
            ];
...
只输出 JSON，不要有其他文字。`;
    ...
    if (remainingCount > 0) {
        ...
        const shuffled = [...available].sort(() => Math.random() - 0.5).slice(0, remainingCount);
    ...
    async callAIForForumComment(identity, post) {
        const chat = this.getChat(identity.id);
